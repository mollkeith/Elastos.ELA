...
func RunPrograms(data []byte, programHashes []common.Uint168, programs []*Program) error {
	...
	for i, program := range programs {
		...
		// PrefixCrossChain addresses are derived from the sidechain genesis
		// redeem script, not from the arbiter/schnorr code used to spend them.
		// Therefore the codeHash == ownerHash binding enforced below must not
		// apply here. Spending of CrossChain UTXOs is restricted by
		// checkTransactionCrossChainUTXO (tx-type whitelist) after
		// ProhibitTransferFromCrossChainHeight.
