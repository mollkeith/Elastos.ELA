...
const (
	...
	// in the pool and still be retained. Without it, appendBlock stores every
	// block that passes CheckBlockSanity, whose only work gate is
	// CheckProofOfWork against PowLimit (2^255-1, which is free), at any height,
	// and the retention sweep drops only entries below the tip, so blocks minted
	// at arbitrary future heights stay for the life of the process. Nothing
	// further above the tip than the pool's own retention window can be confirmed
	// from here anyway, and a genuinely connectable orphan is held by the chain's
	// orphan pool (AddOrphanBlock/ProcessOrphans), not by this map.
	retainAheadCount = cachedCount

	// maxOrphanConfirms bounds the number of confirms retained for blocks that
	// are not in the pool. appendConfirm stores the confirm before confirmBlock
	// runs, and the sweep visits only confirms keyed by blocks that are in the
	// pool, so without this bound a confirm whose block never arrives stays for
	...
	// BYTES.
	//
	// The height-based retention window bounds the pool by height, but bm.blocks is
	// keyed by hash, so nothing else bounds how many distinct blocks can sit at one
	// retained height. Each has a different hash, each is inside [floor, ceiling],
	// and pruneLocked therefore retains all of them until the tip moves past that
	// height, which is roughly three block intervals in the steady state and forever
	// on a chain that has stopped connecting blocks, because the sweep's only
	// production driver is the block-connected path (elanet/netsync/manager.go
	// CleanFinalConfirmedBlock). Minting such a block is free: appendBlock's only
	// work gate is CheckBlockSanity, whose CheckProofOfWork runs against PowLimit
	// (2^255-1).
	//
	// The legitimate occupancy of the retention window is
	// cachedCount+retainAheadCount+1 = 13 heights, and the retained mainchain has
	// exactly one height that ever held two distinct blocks, so 64 entries is
	// roughly 5x the largest occupancy real history can justify. The byte budget is
	// the bound that actually matters, since a count cap alone still concedes
	// maxPooledBlocks x MaxBlockContextSize of RAM.
	maxPooledBlocks = 64
...
type BlockPool struct {
	...
	// len(confirmAge) is also the orphan-confirm count that maxOrphanConfirms
	// bounds.
	...
	// putBlockLocked and dropBlockLocked so they cannot drift.
	...
	// rather than one per evicted block.
	shedding    bool
...
func (bm *BlockPool) appendBlock(dposBlock *types.DposBlock) (bool, bool, error) {
	...
	// Admission bound, and it must be checked before CheckBlockSanity so an
	// out-of-window block costs neither the serialization nor the merkle work.
	// This is the only admission path that may refuse a block outright, and it is
	// safe to do so precisely here: a block with no confirm can never reach
	// Chain.ProcessBlock from this function, because confirmBlock below fails on
	...
	// enforces on retention, so this rejects only blocks the very next sweep would
	// delete anyway, and unlike the sweep it holds while the chain is stalled,
	// which is when the sweep never runs.
	//
	// Debug, not Info: the netsync manager already logs every AddDposBlock error
	// at Warn, and an attacker sets the rate at which this fires.
...
func (bm *BlockPool) appendBlockAndConfirm(dposBlock *types.DposBlock) (bool, bool, error) {
	...
	// add block
	//
	// No height rejection here, by design. This path feeds Chain.ProcessBlock,
	// whose isOrphan result drives the netsync manager's PushGetBlocksMsg ancestor
	// recovery, so a node that is far behind must still be able to hand a far-ahead
	// block and confirm to the chain. The pool is bounded by shedding instead,
	// inside putBlockLocked.
...
func (bm *BlockPool) appendConfirm(confirm *payload.Confirm) (
	...
	if err != nil {
		// The confirm stays in bm.confirms so that a block which arrives after its
		// confirm can still be confirmed, but from here on it is tracked and bounded
...
func (bm *BlockPool) AddToBlockMap(block *types.Block) {
	...
	// This path performs no validation at all, so it is bounded by shedding
...
func (bm *BlockPool) AddToConfirmMap(confirm *payload.Confirm) {
	...
	bm.confirms[confirm.Proposal.BlockHash] = confirm
	// Bound this entry too when its block is not in the pool.
	bm.trackOrphanConfirmLocked(confirm.Proposal.BlockHash)
}
...
//
// This is retention only. It never rejects an incoming block or confirm, so it
// cannot change which blocks or transactions the node accepts: an evicted block
// that later becomes relevant is simply re-requested on the next announcement,
// and a connectable orphan lives in the chain's own orphan pool rather than here.
func (bm *BlockPool) pruneLocked(height uint32) {
	...
	// Report recovery once, on the way back down.
	...
	// Age out confirms with no block in the pool. Retaining them for a while is
	// intended, since a confirm may legitimately arrive before its block, but one
	// whose block has not shown up within the pool's own retention window is dead
	// weight.
...
// admission bound must agree on this window: an admission rule stricter than
// retention would drop blocks the sweep would have kept, and one looser would
// admit blocks the sweep deletes at the next connect, so both read it from here.
//
// reference-cachedCount underflows for the first cachedCount heights and wraps
// to about 2^32, which drops the entire pool, including the block being
// connected, during genesis bootstrap; the floor below is computed so it cannot.
...
// putBlockLocked inserts block into the pool and enforces the count and byte
// caps. The caller must hold bm's write lock.
//
// It never refuses the insert. Two of the three admission paths
// (appendBlockAndConfirm, AddToBlockMap) hand the block to Chain.ProcessBlock,
// whose isOrphan result drives the netsync manager's PushGetBlocksMsg recovery,
// so refusing there would convert a retention bug into a sync-liveness bug.
// Instead the pool sheds the entry furthest from the reference height, which is
// exactly the entry pruneLocked would delete first and never the tip-adjacent
// block the node is actually trying to connect.
//
...
// whether anything was evicted. The caller must hold bm's write lock.
...
// It exists so the pool bounds can be asserted from outside the package.
