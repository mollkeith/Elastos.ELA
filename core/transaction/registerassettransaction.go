...
// HeightVersionCheck rejects RegisterAsset at/above the recovery gate.
// RegisterAsset spends real inputs but GetTxReference (chainstore.go:180) and the
// UnspentIndex skip it, so its inputs are never retired, which allows a fee-mint or
// double-spend under DPoS. Custom assets are deprecated on the ELA mainchain
// (GetAssetByHash is hardcoded to ELA and marked for removal), so RegisterAsset has
// no legitimate post-genesis use. Height-gated: below the gate the legacy behavior is
// preserved verbatim for replay-safety; at/above it the type is refused.
