...
//
// This test drives the REAL core/transaction.checkTransactionSignature — the SECOND of the
// two callers the audit mutated (the first is blockchain/txvalidator.go, covered by
// blockchain/wiring_runprograms_test.go). Both had to be mutated together to disarm the
// gate, so both need their own wiring test. It asserts the production function
...
// txUnknownStandardCode is Standard-prefixed by construction but is neither a Standard,
// Schnorr, nor MultiSig script — the exact F-049 shape that used to fall through
// RunPrograms with no signature check at all.
var txUnknownStandardCode = []byte{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x11, 0x22, 0x33, 0x44, 0x55}

// f049Tx builds a transfer whose single input is parked at the unknown-standard address.
func f049TxChecker(t *testing.T) (interfaces.Transaction, map[*common2.Input]common2.Output) {
	t.Helper()
	if contract.IsStandard(txUnknownStandardCode) || contract.IsSchnorr(txUnknownStandardCode) ||
		contract.IsMultiSig(txUnknownStandardCode) {
		t.Fatal("precondition: the code must be none of Standard/Schnorr/MultiSig")
	}
	ph := common.ToProgramHash(byte(contract.PrefixStandard), txUnknownStandardCode)
	...
	tx := functions.CreateTransaction(
		...
		0, []*program.Program{{Code: txUnknownStandardCode, Parameter: []byte{}}})

	refs := map[*common2.Input]common2.Output{in: {AssetID: core.ELAAssetID, Value: 2, ProgramHash: *ph}}
	return tx, refs
}

// TestWiringTxCheckerRunProgramsReceivesTheBlockHeight is the F-049/F-091 threading proof.
//
// MUTATION PROOF: change the blockchain.RunPrograms call in
// core/transaction.checkTransactionSignature to pass a
...
func TestWiringTxCheckerRunProgramsReceivesTheBlockHeight(t *testing.T) {
	tx, refs := f049TxChecker(t)
...
// TestWiringTxCheckerRunProgramsReceivesTheStrictMoneyHeight pins the OTHER argument: the gate
// itself must be the campaign gate the caller supplies, not a constant. Moving the gate
// moves the decision boundary.
func TestWiringTxCheckerRunProgramsReceivesTheStrictMoneyHeight(t *testing.T) {
	tx, refs := f049TxChecker(t)
